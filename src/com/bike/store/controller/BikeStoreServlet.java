	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("insert".equals(action)) {
			...
			try {
				...
				// 經緯度
				Double store_longitute = null;
				Double store_latitute = null;
				try {
					store_longitute = new Double(request.getParameter("store_longitute"));
					store_latitute = new Double(request.getParameter("store_latitute"));
				} catch (NumberFormatException ce) {
					errorMsgs.put("location", "請確認座標");
				}
				...
				InputStream in = store_photo.getInputStream();
				...
				if (!errorMsgs.isEmpty()) {
					String url = "/back-end/bikeStore/addBikeStore.jsp";
		...
		// 店家後台選擇
		if ("bikeStoreOwner".equals(action)) {
			// -- 請求參數
			String sq_bike_store_id = request.getParameter("sq_bike_store_id");
			BikeStoreService BikeStoreService = new BikeStoreService();
			BikeStoreVO BikeStoreVO = BikeStoreService.findByPrimaryKey(sq_bike_store_id);
			request.setAttribute("BikeStoreVO", BikeStoreVO);
			RequestDispatcher successView = request.getRequestDispatcher("/back-end/bike/bikeStoreOwner.jsp");
			successView.forward(request, response);
		}
		
		
		if ("getOne_For_Update".equals(action) || "getOne_For_Update_bikeStoreOwner".equals(action)) {
			String sq_bike_store_id = request.getParameter("sq_bike_store_id");
			String bike_store_name = request.getParameter("bike_store_name");
			...
			if("getOne_For_Update".equals(action)) {
				...
				String url = request.getContextPath() + "/back-end/bike/bikeStoreOwner.jsp";
				response.sendRedirect(url);
			}

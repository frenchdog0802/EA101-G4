...
<div class="container login-container">
	...
	<div class="row">
		<div class="col-md-6 login-form-1">
			...
			<form action="<%=request.getContextPath()%>/login/stafflogin.do"
				method="POST">
				<div class="form-group">
					<input type="text" class="form-control" name="staffAccount"
						placeholder="Your Account *" value="${param.staffAccount}" />
					<!--errorMsg -->
				...
				<div class="form-group">
					<input type="password" class="form-control" name="staffPassword"
						placeholder="Your Password *" value="${param.staffPassword}" />
					<!--errorMsg -->
					<span class="small text-danger errorMsgstaffPassword d-block">${errorMsgs.staffPassword}</span>
				</div>
				<div class="form-group">
					<input type="hidden" name="action" value="staffLogin"> <input
						type="submit" class="btnSubmit" value="登入" />
				</div>
			</form>
		</div>
		<div class="col-md-6 login-form-2">
			...
			<form action="<%=request.getContextPath()%>/login/stafflogin.do"
				method="POST">
				<div class="form-group">
					<input type="text" class="form-control" name="storeAccount"
						placeholder="Your Account *" value="${param.storeAccount}" />
					<!--errorMsg -->
					<span class="small text-danger errorMsgstoreAccount d-block">${errorMsgs.storeAccount}</span>
				</div>
				<div class="form-group">
					<input type="password" class="form-control" name="storePassword"
						placeholder="Your Password *" value="${param.storePassword}" />
					<!--errorMsg -->
					<span class="small text-danger errorMsgstorePassword d-block">${errorMsgs.storePassword}</span>
				</div>
				<div class="form-group">
					<input type="hidden" name="action" value="storeLogin"> <input
						type="submit" class="btnSubmit" value="登入" />
				</div>
			</form>
		</div>
